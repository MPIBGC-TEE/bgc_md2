...
u_func_numerical = numerical_function_from_expression(
    ...
    G_emanuel * exp(0*time_symbol), # Dummy expression
...
ax.plot(btt_median)
fig.savefig("Figures/python_btt_median.pdf")
# -

btt_median_from_csv = np.loadtxt(
    "backward_transit_time_mean_and_median.csv",
    skiprows=1,
    delimiter=",",
    usecols=[2]
)

np.argmax(np.diff(btt_median_from_csv[2:]) > 0)

# +
fig, axes = plt.subplots(figsize=(12, 6))
ax = axes

ax.plot(btt_median_from_csv)
# -

plt.plot(smr.external_output_vector.sum(1))

pool_age_dens_from_csv = np.loadtxt(
    "Emmanuel_age_dens.csv",
    skiprows=1,
    delimiter=",",
)
pool_age_dens_from_csv

# +
pool_nr = -1

ages = pool_age_dens_from_csv[:, 0][pool_age_dens_from_csv[:, 2] == pool_nr]
times = pool_age_dens_from_csv[:, 1][pool_age_dens_from_csv[:, 2] == pool_nr]
vals = pool_age_dens_from_csv[:, 3][pool_age_dens_from_csv[:, 2] == pool_nr]

# +
t = 1851

x = pool_age_dens_from_csv[pool_age_dens_from_csv[:, 2] == pool_nr]
#x = x[:, [0, 1, 3]]
x = x[:, x[:, 1] == ti]
x
...
x = pd.read_csv("Emmanuel_age_dens.csv")
x.columns
...
x = pd.read_csv("Emmanuel_age_dens.csv", header=0, )

x = pd.read_csv("Emmanuel_age_dens.csv", header=0, )
x.columns = ["age", "time", "pool", "value"]
for ti in np.arange(135, 141, 1):
    y = x.loc[x["pool"] == 4]
    y = y.loc[x["time"] == 1851+ti]
    plt.plot(y["age"], y["value"], label=str(ti))


data_times = np.arange(smr.times[0], smr.times[-1] + 0.1, 0.1)
data_times

#Bs = smr.fake_discretized_Bs(data_times)
Bs = smr.fake_discretized_Bs()
Bs.shape

#t = data_times[0]
#f = smr.external_input_vector_func()
#Us = np.array([f(t) for t in data_times])
#Us.shape
Us = smr.external_input_vector

from CompartmentalSystems.discrete_model_run import DiscreteModelRun as DMR

dmr = DMR.from_Bs_and_net_Us(smr.start_values, smr.times, Bs, Us[:-1])

plt.plot(dmr.solve().sum(1))
plt.plot(smr.solve().sum(1))
