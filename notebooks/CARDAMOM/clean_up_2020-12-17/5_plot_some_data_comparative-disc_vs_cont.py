...
# # Compare discrete vs continuous results with plots
...
for var_name, ax in zip(var_names, axes[0]):
    ...
    ax.set_title(var_name + " (discrete)")
...
for var_name, ax in zip(var_names, axes[1]):
    ...
    ax.set_title(var_name + " (continuous)")
...
for var_name, ax in zip(var_names, axes[2]):
    ...
    ax.set_title(var_name + " (discrete minus continuous)")
...
#        cbar_kwargs={"label": ds1[var_name].attrs["units"]},
    ...
    ax.set_title(var_name + " (discrete-continuous)/discrete*100")
...
plt.suptitle("Pool age (discrete minus continuous)")
plt.tight_layout()
plt.draw()
...
plt.suptitle("Pool age (dicrete-continuous)/discrete*100")
#plt.tight_layout()
plt.draw()
# -
...
for var_name_pair, ax in zip(var_name_pairs, axes.flatten()):
    for var_name in var_name_pair:
        ds1[var_name].isel(lat=lat, lon=lon, prob=prob).plot(
            ax=ax,
            label=var_name + " (discrete)",
            ls="-"
        )
        ds2[var_name].isel(lat=lat, lon=lon, prob=prob).plot(
            ax=ax,
            label=var_name + " (continuous)",
            ls="--"
        )
    ax.legend()
...
axes[0].set_title("Total C stocks (discrete)")
...
axes[1].set_title("Total C stocks (continuous)")
...
axes[2].set_title("Total C stocks (discrete minus continuous)")
...
global_mean_system_age_1.plot(ax = ax, label="Global C mean age (discrete)", c="blue", ls="-")
global_mean_system_age_2.plot(ax = ax, label="Global C mean age (continuous)", c="blue", ls="--")

global_avg_system_age_median_1.plot(ax = ax, label="Global average system age median (discrete)", c="orange", ls="-")
global_avg_system_age_median_2.plot(ax = ax, label="Global average system age median (continuous)", c="orange", ls="--")

global_mean_btt_1.plot(ax=ax, label="Global mean backward transit time (discrete)", c="red", ls="-")
global_mean_btt_2.plot(ax=ax, label="Global mean backward transit time (continuous)", c="red", ls="--")

global_avg_btt_median_1.plot(ax=ax, label="Global average backward transit time median (discrete)", c="green", ls="-")
global_avg_btt_median_2.plot(ax=ax, label="Global average backward transit time median (continuous)", c="green", ls="--")
...
for variable_name, ax, title in zip(variable_names, axes.flatten(), titles):
    f(ds1[variable_name]).plot(ax=ax, label="discrete", c="red")
    f(ds2[variable_name]).plot(ax=ax, label="continuous", c="blue")
    ax.set_title(title)
    ax.set_ylabel("yr")
    ax.legend()
