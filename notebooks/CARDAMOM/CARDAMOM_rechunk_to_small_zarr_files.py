...
#    zarr_dirname = "%02d_%03.2f_%03.2f" % (chunk_ds.prob, chunk_ds.lat, chunk_ds.lon)
#    print(zarr_dirname)
#    chunk_ds.to_netcdf(data_folder + filestem + "small_netcdf/" + filename)
#    chunk_ds.to_zarr(data_folder + filestem + "zarr_version/" + zarr_dirname)
#    return chunk_ds
...
# %%time
for name, value in ds_rechunked.variables.items():
    zarr_dir_name = data_folder + filestem + "zarr_version/" + str(name)
    print(zarr_dir_name)
    dask.array.asarray(value.data).to_zarr(zarr_dir_name)
