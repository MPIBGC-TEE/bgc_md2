...
class TestGraphVizualization(InDirTest):
    def setUp(self):
        ...
        self.computer_aliases={v.__name__:ascii_lowercase[i] for i,v in enumerate(self.bgc_computers)}
        allVars=all_mvars(self.bgc_computers)
        self.mvar_aliases={
            name:ascii_uppercase[i] for i,name in 
            enumerate(
                sorted(
                    map(lambda v:v.__name__, allVars)
                )
            )
        }
    ...
    def test_ComputerSetMultiGraph_matplotlib_bgc_md2(self):
        ...
        fig=plt.figure(figsize=(20,20))
        ax=fig.add_subplot(1,1,1)
        draw_ComputerSetMultiDiGraph_matplotlib(ax,spsg,self.mvar_aliases,self.computer_aliases)
        fig.savefig("SetMultiGraph.pdf")


    def test_update_generator_bgc_md2(self):
        fig=plt.figure()
        draw_update_sequence(self.bgc_computers,8,fig,self.mvar_aliases,self.computer_aliases)
        fig.savefig('c1.pdf')



    @skip("very immature and nearly manual, but maybe one of the possibilities to make the connections clickable?")
