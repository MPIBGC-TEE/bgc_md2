...
def MDO_3pools_3layers():
    ...
    data = (np.arange(len(time) * nr_layers) + 0.1).reshape((len(time), nr_layers))
    var[...] = data
    var.units = "gC/m^3"
    var.cell_methods = "time: instantaneous"

    var = ds.createVariable("SOILC", "f8", ("time", "level"))
    data = (np.arange(len(time) * nr_layers) + 0.2).reshape((len(time), nr_layers))
    var[...] = data.copy()
    var.units = "gC/m^3"
    var.cell_methods = "time: instantaneous"
    ...
    data = np.arange(len(time) * nr_layers).reshape((len(time), nr_layers)) * 1e-03
    var[...] = data
    var.units = "gC/m^3/s"
    ...
    data = np.arange(len(time) * nr_layers).reshape((len(time), nr_layers)) * 1e-04
    var[...] = data
    var.units = "gC/m^2/s"
    var.cell_methods = "time: mean"
...
def MDO_3pools_nolayers_nogrid():
    ...
    pool_structure = [
        ...
        {"pool_name": "Litter", "stock_var": "LITRC",},
        {"pool_name": "Soil", "stock_var": "SOILC",},
    ]
...
def MDO_3pools_3layers_discretizable():
    ...
    ds = Dataset("test_dataset_discretizable1.nc", "w", diskless=True, persist=False)
    ds.createDimension("time", len(time))
    ds.createDimension("level", nr_layers)
