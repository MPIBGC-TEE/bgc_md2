...
class TestModelDataObject(unittest.TestCase):
    def setUp(self):
        ...
        ## incomplete models
        mdos_inc = [MDO_3pools_nolayers_nogrid(), MDO_3pools_3layers_nogrid()]
    ...
    def test_readVariable(self):
        ...
        var = readVariable(dataset=ds, variable_name="CWDC", nr_layers=2, data_shift=1)
        res2 = np.array(
            [
                [3.0, 4.0],
                [6.0, 7.0],
    ...
    def test_init(self):
        ...
        self.assertTrue(np.all(mdo.time_agg.data == np.array([0, 2, 4, 6, 8, 9])))

        xs = mdo.load_stocks(func=getStockVariable_from_Density, data_shift=0)
    ...
    def test_load_stocks(self):
        ...
        res = xs.data[:, pool_nr]
        res2 = 3 * np.array([2.2, 8.2, 14.2, 20.2, 26.2, 29.2])
        self.assertTrue(np.allclose(res, res2))
    ...
    def test_load_horizontal_fluxes(self):
        ...
        res2 = np.array(
            [
                [[777.6, 0.0, 0.0], [0.0, 1900.8, 0.0], [0.0, 0.0, 3369.6]],
                [[1814.4, 0.0, 0.0], [0.0, 3974.4, 0.0], [0.0, 0.0, 6480.0]],
                [[2851.2, 0.0, 0.0], [0.0, 6048.0, 0.0], [0.0, 0.0, 9590.4]],
                [[3888.0, 0.0, 0.0], [0.0, 8121.6, 0.0], [0.0, 0.0, 12700.8]],
                [[2332.8, 0.0, 0.0], [0.0, 4838.4, 0.0], [0.0, 0.0, 7516.8]],
            ]
        )
    ...
    def test_load_vertical_fluxes(self):
        ...
        res2 = np.array([[0.0, 285.12, 0.0], [2851.2, 0.0, 302.4], [0.0, 3024.0, 0.0]])
        ...
        res2 = np.array([[0.0, 302.4, 0.0], [3024.0, 0.0, 319.68], [0.0, 3196.8, 0.0]])
        ...
        for nr, mdo in enumerate(self.mdos_inc):
            ...
            self.assertTrue(np.all(runoffs_down.data.shape == runoffs_shapes[nr]))
