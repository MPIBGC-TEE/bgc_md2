...
MODULE cable_output_module
  ...
  TYPE out_varID_type ! output variable IDs in netcdf file
    INTEGER :: SWdown, LWdown, Wind, Wind_E, PSurf,                       &
                    ...
                    RadT, VegT, cisun, cisha, cx1sun, cx1sha, cx2sun, cx2sha,  &
                    Ebal, Wbal, AutoResp, LeafResp, RootResp, HeteroResp, GPP, NPP, LAI, &
                    Nsoilmin, Nsnet, Nmindep, Nminfix, Nminloss, Nminleach, Nupland, &
...
!    REAL(KIND=4), POINTER, DIMENSION(:) :: DelIntercept
    ...
    REAL(KIND=4), POINTER, DIMENSION(:) :: LeafResp   ! 51 autotrophic
                                                      ! respiration [umol/m2/s]
    REAL(KIND=4), POINTER, DIMENSION(:) :: RootResp ! 50 heterotrophic
                                                      ! respiration [umol/m2/s]
    REAL(KIND=4), POINTER, DIMENSION(:) :: HeteroResp ! 50 heterotrophic
                                                      ! respiration [umol/m2/s]
    REAL(KIND=4), POINTER, DIMENSION(:) :: SnowDepth  ! actual depth of snow in
                                                      ! [m]
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nsoilmin  ! Soil mineral N gN/m2
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nsnet     ! Soil net N mineralisation gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nmindep   ! N deposition gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nminfix   ! N fixation gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nminloss  ! mineral N loss gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nminleach ! mineral N leaching gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: Nupland ! mineral N uptake by vegetation gN/m2/day
    REAL(KIND=4), POINTER, DIMENSION(:) :: cisun   ! 51 intercellular CO2 concentration on sun leaf
    REAL(KIND=4), POINTER, DIMENSION(:) :: cisha   ! 52 intercellular CO2 concentration on shadow leaf
    REAL(KIND=4), POINTER, DIMENSION(:) :: cx1sun  ! 53 enzyme kinetics on sun leaf
    REAL(KIND=4), POINTER, DIMENSION(:) :: cx1sha  ! 54 enzyme kinetics on shadow leaf
    REAL(KIND=4), POINTER, DIMENSION(:) :: cx2sun  ! 55 CO2 compensation point on sun leaf
    REAL(KIND=4), POINTER, DIMENSION(:) :: cx2sha  ! 56 CO2 compensation point on shadow leaf
    ! Non-Alma variables
...
CONTAINS
  ...
  SUBROUTINE open_output_file(dels, soil, veg, bgc, rough)
    ...
    IF(output%veg .OR. output%cisun) THEN
       CALL define_ovar(ncid_out, ovid%cisun, 'cisun', '-',                      &
                        'intercellular CO2 concentration on sun leaf', patchout%cisun,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cisun(mp))
       out%cisun = 0.0 ! initialise
    END IF
    IF(output%veg .OR. output%cisha) THEN
       CALL define_ovar(ncid_out, ovid%cisha, 'cisha', '-',                      &
                        'intercellular CO2 concentration on shadow leaf', patchout%cisha,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cisha(mp))
       out%cisha = 0.0 ! initialise
    END IF
    IF(output%veg .OR. output%cx1sun) THEN
       CALL define_ovar(ncid_out, ovid%cx1sun, 'cx1sun', '-',                      &
                        'enzyme kinetics on sun leaf', patchout%cx1sun,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cx1sun(mp))
       out%cx1sun = 0.0 ! initialise
    END IF
    IF(output%veg .OR. output%cx1sha) THEN
       CALL define_ovar(ncid_out, ovid%cx1sha, 'cx1sha', '-',                      &
                        'enzyme kinetics on shadow leaf', patchout%cx1sha,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cx1sha(mp))
       out%cx1sha = 0.0 ! initialise
    END IF
    IF(output%veg .OR. output%cx2sun) THEN
       CALL define_ovar(ncid_out, ovid%cx2sun, 'cx2sun', '-',                      &
                        'CO2 compensation point on sun leaf', patchout%cx2sun,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cx2sun(mp))
       out%cx2sun = 0.0 ! initialise
    END IF
    IF(output%veg .OR. output%cx2sha) THEN
       CALL define_ovar(ncid_out, ovid%cx2sha, 'cx2sha', '-',                      &
                        'CO2 compensation point on shadow leaf', patchout%cx2sha,       &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%cx2sha(mp))
       out%cx2sha = 0.0 ! initialise
    END IF
    ...
    END IF
    IF(output%carbon .OR. output%RootResp) THEN
       CALL define_ovar(ncid_out, ovid%RootResp, 'RootResp', 'umol/m^2/s', &
                        'Root respiration', patchout%RootResp,      &
                        'dummy', xID, yID, zID, landID, patchID, tID)
    ...
    IF(output%casacnp .OR. output%Nsnet) THEN
       CALL define_ovar(ncid_out, ovid%Nsnet, 'Nsnet', 'gN/m2/day',               &
                        'Soil net N mineralisation', patchout%Nsnet,           &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%Nsnet(mp))
       out%Nsnet = 0.0 ! initialise
    END IF

    IF(output%casacnp .OR. output%Nmindep) THEN
       CALL define_ovar(ncid_out, ovid%Nmindep, 'Nmindep', 'gN/m2/day',               &
                        'N deposition', patchout%Nmindep,           &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%Nmindep(mp))
       out%Nmindep = 0.0 ! initialise
    END IF

    IF(output%casacnp .OR. output%Nminfix) THEN
       CALL define_ovar(ncid_out, ovid%Nminfix, 'Nminfix', 'gN/m2/day',               &
    ...
    IF(output%casacnp .OR. output%Nminloss) THEN
       CALL define_ovar(ncid_out, ovid%Nminloss, 'Nminloss', 'gN/m2',               &
                        'Soil mineral nitrogen loss', patchout%Nminloss,           &
                        'dummy', xID, yID, zID, landID, patchID, tID)
    ...
    IF(output%casacnp .OR. output%Nminleach) THEN
       CALL define_ovar(ncid_out, ovid%Nminleach, 'Nminleach', 'gN/m2',               &
                        'Soil mineral nitrogen leaching', patchout%Nminleach,           &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%Nminleach(mp))
       out%Nminleach = 0.0 ! initialise
    END IF

    IF(output%casacnp .OR. output%Nupland) THEN
       CALL define_ovar(ncid_out, ovid%Nupland, 'Nupland', 'gN/m2',               &
                        'Vegetation N uptake', patchout%Nupland,           &
                        'dummy', xID, yID, zID, landID, patchID, tID)
       ALLOCATE(out%Nupland(mp))
       out%Nupland = 0.0 ! initialise
    END IF
...
!       print*,'out_timestep',out_timestep
    ...
    ! cisun: intercellular CO2 concentration on sun leaf
    IF(output%veg .OR. output%cisun) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cisun = out%cisun + REAL(canopy%cisun, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cisun = out%cisun / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cisun, 'cisun', out%cisun, &
                          ranges%cisun, patchout%cisun, 'default', met)
          ! Reset temporary output variable:
          out%cisun = 0.0
       END IF
    END IF
    ! cisha: intercellular CO2 concentration on shadow leaf
    IF(output%veg .OR. output%cisha) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cisha = out%cisha + REAL(canopy%cisha, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cisha = out%cisha / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cisha, 'cisha', out%cisha, &
                          ranges%cisha, patchout%cisha, 'default', met)
          ! Reset temporary output variable:
          out%cisha = 0.0
       END IF
    END IF
    ! cx1sun: enzyme kinetics on sun leaf
    IF(output%veg .OR. output%cx1sun) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cx1sun = out%cx1sun + REAL(canopy%cx1sun, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cx1sun = out%cx1sun / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cx1sun, 'cx1sun', out%cx1sun, &
                          ranges%cx1sun, patchout%cx1sun, 'default', met)
          ! Reset temporary output variable:
          out%cx1sun = 0.0
       END IF
    END IF
    ! cx1sha: enzyme kinetics on shadow leaf
    IF(output%veg .OR. output%cx1sha) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cx1sha = out%cx1sha + REAL(canopy%cx1sha, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cx1sha = out%cx1sha / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cx1sha, 'cx1sha', out%cx1sha, &
                          ranges%cx1sha, patchout%cx1sha, 'default', met)
          ! Reset temporary output variable:
          out%cx1sha = 0.0
       END IF
    END IF
    ! cx2sun: CO2 compensation point on sun leaf
    IF(output%veg .OR. output%cx2sun) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cx2sun = out%cx2sun + REAL(canopy%cx2sun, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cx2sun = out%cx2sun / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cx2sun, 'cx2sun', out%cx2sun, &
                          ranges%cx2sun, patchout%cx2sun, 'default', met)
          ! Reset temporary output variable:
          out%cx2sun = 0.0
       END IF
    END IF
    ! cx2sha: CO2 compensation point on shadow leaf
    IF(output%veg .OR. output%cx2sha) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%cx2sha = out%cx2sha + REAL(canopy%cx2sha, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%cx2sha = out%cx2sha / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%cx2sha, 'cx2sha', out%cx2sha, &
                          ranges%cx2sha, patchout%cx2sha, 'default', met)
          ! Reset temporary output variable:
          out%cx2sha = 0.0
       END IF
    END IF
    ...
    IF(output%carbon .OR. output%HeteroResp) THEN
       ...
       END IF
    END IF
    ! RootResp: root respiration [umol/m^2/s]
    IF(output%carbon .OR. output%RootResp) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%RootResp = out%RootResp + REAL(canopy%frpr / 1.201E-5, 4)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%RootResp = out%RootResp / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%RootResp,             &
                          'RootResp', out%RootResp, ranges%RootResp,     &
                          patchout%RootResp, 'default', met)
          ! Reset temporary output variable:
          out%RootResp = 0.0
       END IF
    END IF
    ...
    ! Nsnet: net mineralisation [gN/m2/day]
    IF(output%casacnp .OR. output%Nsnet) THEN
       ! Add current timestep's value to total of temporary output variable:
    ...
    ! Nmindep: N deposition [gN/m2/day]
    IF(output%casacnp .OR. output%Nmindep) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Nmindep = out%Nmindep + REAL(casaflux%Nmindep)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%Nmindep = out%Nmindep / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%Nmindep, 'Nmindep', out%Nmindep,    &
                          ranges%Nmindep, patchout%Nmindep, 'default', met)
          ! Reset temporary output variable:
          out%Nmindep = 0.0
       END IF
    END IF

    ! Nminfix: N fixation [gN/m2/day]
    IF(output%casacnp .OR. output%Nminfix) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Nminfix = out%Nminfix + REAL(casaflux%Nminfix)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%Nminfix = out%Nminfix / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%Nminfix, 'Nminfix', out%Nminfix,    &
                          ranges%Nminfix, patchout%Nminfix, 'default', met)
          ! Reset temporary output variable:
          out%Nminfix = 0.0
       END IF
    END IF

!    print*,'Nminloss',casaflux%Nminloss(2000)
    ! Nminloss: mineral N loss due to mineralization [gN/m2/day]
    IF(output%casacnp .OR. output%Nminloss) THEN
       ! Add current timestep's value to total of temporary output variable:
    ...
    ! Nminleach: mineral N loss due to leaching [gN/m2/day]
    IF(output%casacnp .OR. output%Nminleach) THEN
       ! Add current timestep's value to total of temporary output variable:
    ...
    ! Nupland: vegetation N uptake [gN/m2/day]
    IF(output%casacnp .OR. output%Nupland) THEN
       ! Add current timestep's value to total of temporary output variable:
       out%Nupland = out%Nupland + REAL(casaflux%Nupland)
       IF(writenow) THEN
          ! Divide accumulated variable by number of accumulated time steps:
          out%Nupland = out%Nupland / REAL(output%interval, 4)
          ! Write value to file:
          CALL write_ovar(out_timestep, ncid_out, ovid%Nupland, 'Nupland', out%Nupland,    &
                          ranges%Nupland, patchout%Nupland, 'default', met)
          ! Reset temporary output variable:
          out%Nupland = 0.0
       END IF
    END IF
