...
# +
def from_networkx(graph, layout, **kwargs):
        '''
        ...
        Args:
            ...
            layout (function or dict) : a networkx layout function or mapping of node keys to positions.
        ...
        if callable(layout):
            graph_layout = layout(graph, **kwargs)
        else:
            graph_layout = layout

            node_keys = graph_renderer.node_renderer.data_source.data['index']
            if set(node_keys) != set(layout.keys()):
                from warnings import warn
                warn("Node keys in 'layout' don't match node keys in the graph. "
...
def BokehPlot(G):
    ...
    graph = from_networkx(G, graph_layout, scale=1, center=(0,0))
