...
class TestSymbolic(TestCase):
    ...
    def test_aggregate_surrogate_systems(self):
        ...
        with self.subTest(mf=mf):
            ...
            par_dict=gh.make_param_dict(mvs, test_args.cpa, test_args.epa_0)
            func_dict=msh.make_func_dict(mvs,test_args.dvs, test_args.cpa, test_args.epa_0)
            X0 = msh.numeric_X_0(mvs,test_args.dvs,test_args.cpa,test_args.epa_0).reshape(-1)
            #from IPython import embed;embed()
            mvs=mvs.update({
                NumericParameterization(
                    par_dict=par_dict,
            ...
            def sub_system_results(svt):
                ...
                # subsystem
                outer_pools=set(mvs.get_StateVariableTuple()).difference(sv_set)
                ...
                sub_mvs = CMTVS(
                    {
                        ...
                        NumericParameterization(
                            par_dict=par_dict,
                            func_dict={ **func_dict, **outer_pool_funcs}
                        ),
                        NumericStartValueArray(X0[inds]),
                        NumericSimulationTimes(times)
                    },
                    mvs.computers
                )
                sub_smr=sub_mvs.get_SmoothModelRun()
                sub_a_dens_function=lambda a: a_dens_function(a)[inds]    
                sub_start_mean_age_vec = start_mean_age_vec[inds] 
                #age_densities = p(ages)
                mean_btts= sub_smr.backward_transit_time_moment(
                    order=1,
                    start_age_moments=sub_start_mean_age_vec.reshape(1,len(svt))
                )
                return mean_btts
                
            veg_res, soil_res= map(
                sub_system_results,
                (
                    mvs.get_VegetationCarbonStateVariableTuple(),
                    mvs.get_SoilCarbonStateVariableTuple(),
                )
            )
            # compute the start age distribution
            ...
            ax.plot(
                ...
                color=color_dict["veg"],
                marker=marker_dict["btt"],
				label="mean veg subsystem backward transit time"
            )
            ax.plot(
                times,
				soil_res ,
