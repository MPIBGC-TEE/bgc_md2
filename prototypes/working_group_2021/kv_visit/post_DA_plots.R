...
df_j<-read.csv(paste0(dataPath,"/visit_demo_da_j_aa.csv"))
df_j<-df_j[-c(1)]
df_j<-t(df_j)
df_j<-as.data.frame(df_j)


#df<-df[round(0.5*dim(df)[1]):dim(df)[1],]
#df_j<-df_j[round(0.5*dim(df_j)[1]):dim(df_j)[1],]
...
epa_min_J<-as.list(df[df_j[2]==min(min(df_j[2])),])
...
{ 
plot(df_j, type="l", col="green", xlab="iterations", ylab="cost value", main="Cost functions over time",
     xlim=c(0,max(max(df_j[1]),max(df_j_demo[1]), max(df_j_autostep[1]))), 
     ylim=c(min(min(df_j[2]),min(df_j_demo[2])),max(max(df_j[2]),max(df_j_demo[2]))))
...
df_j1<-df_j[round(0.9*length(df_j$V2)):length(df_j$V2),]
df_j_demo1<-df_j_demo[round(0.9*length(df_j_demo$V2)):length(df_j_demo$V2),]
df_j_autostep1<-df_j_autostep[round(0.9*length(df_j_autostep$V2)):length(df_j_autostep$V2),]

jpeg(paste0(dataPath,'/cost_functions_last_10_percent_', model_name,'_',time_stamp,'.jpeg'), width = 15, height = 8, units = 'in', res = 300)
{
  plot(df_j1, type="l", col="green", xlab="iterations", ylab="cost value", main="Last 10% of accepted cost functions",
       xlim=c(min(min(df_j1[1]),min(df_j_demo1[1]),min(df_j_autostep1[1])),max(max(df_j1[1]),max(df_j_demo1[1]),max(df_j_autostep1[1]))), 
       ylim=c(min(min(df_j1[,2]),min(df_j_demo1[2]),min(df_j_autostep1[2])),max(max(df_j1[2]),max(df_j_demo1[2]),max(df_j_autostep1[2])))) 
...
{ # plot model output with optimized parameters
  ...
  legend(x = "topleft", legend=c("CMIP-6 Output", "Demo run", "Formal run", "Demo-autostep"),
...
# plot as a .jpeg file - MAXIMUM LIKELIHOOD
...
{ # plot model output with optimized parameters
  ...
  for (i in 1:length(names(sol_median_formal))) {
    ...
    plot(sol_likelihood_demo[[i]][1:N], type="l", col="red", xlab="month",
         ylim=c(min(min(sol_min_J_formal[[i]]),min(obs[[i]])),max(max(sol_min_J_formal[[i]]),max(obs[[i]]))), 
         ylab=names(sol_median_formal)[i], main=names(sol_median_formal)[i])
    lines(obs[[i]], col="blue")
    #lines(sol_mean[[i]], col="orange")
    lines(sol_likelihood_formal[[i]], col="green")
...
# plot as a .jpeg file - Different solutions of 1 mcmc
...
{ # plot model output with optimized parameters
  ...
  for (i in 1:length(names(sol_median_formal))) {
    ...
    plot(sol_median_autostep[[i]][1:N], type="l", col="red", xlab="month",
         ylim=c(min(min(sol_min_J_formal[[i]]),min(obs[[i]])),max(max(sol_min_J_formal[[i]]),max(obs[[i]]))), 
         ylab=names(sol_median_formal)[i], main=names(sol_median_formal)[i])
    lines(obs[[i]], col="blue")
    #lines(sol_mean[[i]], col="orange")
  ...
  plot(0, type = 'n', axes = FALSE, ann = FALSE, main="legend")
  legend(x = "topleft", legend=c("CMIP-6 Output", "Median", "Min Cost", "Max Likelihood"),
         col=c("blue", "red", "green", "orange"), lty=1, cex=1)
