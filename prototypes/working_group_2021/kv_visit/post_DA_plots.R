...
jpeg('posterior_distr_demo_visit.jpeg', width = 15, height = 8, units = 'in', res = 300)
{
...
# visualize parameter distributions with median, mode and min cost function
jpeg('posterior_distr_visit.jpeg', width = 15, height = 8, units = 'in', res = 300)
{
  par(mfrow=c(4, 6)) # make 4x6 plots in 1 window
  for (i in 1:length(df)) {
    hist(df[[i]], breaks=100, col="lightgray", border="lightgray", probability=T, main=par_names[i])
...
jpeg('cost_functions_visit.jpeg', width = 15, height = 8, units = 'in', res = 300)
{
...
sol_init_par<-read.csv(paste0(dataPath,"/sol_init_par.csv"))
...
dat<-read.csv(paste0(dataPath,"/obs.csv")) 
...
names(sol_median)<-c(    "C_leaf",
                         ...
                         "f_veg2litter",
                         'f_litter2som')
jpeg('post_da_simu_visit.jpeg', width = 15, height = 8, units = 'in', res = 300)
{ # plot model output with optimized parameters
  ...
  for (i in 1:length(names(sol_median))) {
    ...
    lines(sol_init_par[[i]], col="purple")
  }
  plot(2, xlim=c(0,1), ylim=c(0,0.9), axes = F, main="legend", ylab="")
  legend(0.1, 0.9, legend=c("CMIP-6 Output", "Median", "Min Cost Function", "Initial parameters"),
         col=c("blue", "red", "green", "purple"), lty=1, cex=1)
