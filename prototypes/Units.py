...
def iterable_unit(exprs):
    ...
    non_zero_exprs = [e for e in filter(lambda exp: 0 != exp, simp_exprs)]
...
class TestUnitExamples(unittest.TestCase):
    ...
    def test_dimensions_of_expressions(self):
        ...
        self.assertTrue(
            dimsys_SI.equivalent_dims(
                SI.get_dimensional_expr(res), SI.get_dimensional_expr(res_val)
            )
        )
    ...
    def test_numerification(self):
        ...
        def v_unit(t):
            ...
            cond_1 = simplify(t) == 0
            cond_2 = dimsys_SI.equivalent_dims(SI.get_dimensional_expr(t), time)
        ...
        def numerify(f_quant, *units):
            def f_num(*num_args):
                target_unit = units[-1]
                u_args = tuple(
                    num_arg * units[ind] for ind, num_arg in enumerate(num_args)
                )
