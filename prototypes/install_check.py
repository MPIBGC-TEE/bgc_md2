...
try:
    ...
    print(
        "\tVoila is not installed! Please install it by running one "
        "of the following:"
    )
...
def version_checker(package_name, version, nbextension=None):
    good_version = any(
        version.startswith(v) for v in version_check_packages[package_name]
    )
    ...
    if not good_version:
        ...
        print(
            "        conda install {}={} # if you use conda".format(
                package_name, newest
            )
        )
...
if required_kernel not in known_kernels:
    print(
        FIX_PREFIX,
        "Please create custom kernel with: ",
