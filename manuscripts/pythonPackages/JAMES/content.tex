...
\title{The biogeochemichal model database \texttt{bgc\_md2} and the \Python\ 
...
This becomes a challenge when they \hmm{What is they?} are implemented in software
...
obscured the ways to obtain them. \hmm{What is ``them''?}  A way to apply the \emph{same} tools to
...
bilinearly \hmm{Quadratically?}, and with it the technical challenges to maintain both collections.
...
Ideally, we want to turn the bilinear \hmm{quadratic?} growth of combinations of models and
...
through the vegetation and soil subsystems.  \hmm{Where exactly is this done? In the notebooks? In which one?}
...
set of edges. \hmm{Just for describing the structure, let's say equivalent to the adjacency matrix, because a graph $G=(V, E)$ is just a pair of vertices and edges.
The compartmental matrix additionally contains numbers, which would lead to at least a graph with weights at the edges. Then the initial condition \vec{x}^0 would still be missing in the graph.}
...
\hmm{Currently it makes no sense to change the notation in each place for me. It seems mostly arbitrary. There are several versions of $x$ and $X$, the matrix sometimes is $B$, sometimes $M$, the input vector is sometimes $u$ (which in the appendix is used to denote the total input), sometimes $I$. Please try to get some system into that, otherwise proof-reading of any formula throughout the manuscript is impossible.}
...
We describe the relationship between graph- and matrix-based descriptions in \Appendixref{appendix:MatrixDerivation}.

We distinguish between different types of compartmental systems, according to linearity and autonomy. If the vector of inputs or the compartmental matrix depend on the vector of states in system \eqref{eq:CompartmentalSystem}, we call it non-linear \hmm{Actually, in the appendix you allow the input to depend on the state vector in a specific way for the system still to be called linear. I would exclude this case. Even though the ODE might still be linear, we can DEFINE the compartmental system to be nonlinear in this case, because as you explain this special case comes with some complications.}, and linear otherwise. Similarly, if the vector of inputs or the compartmental matrix depend on time, we call the system non-autonomous, and autonomous otherwise. 
...
\begin{enumerate}
  ...
  Preferably \hmm{Should we use this word in a hard criterion?} metrics should also be physical properties, \ie have a clearly defined way to measure them at least in principle.
  \item
    \label{enum:meaningful}
  Preferably \hmm{Same question.} metrics should be relevant to answer research questions related to real-world phenomena.
\end{enumerate}
...
Beside the content of pools, the whole system, or subsystems and fluxes between
...
This makes them the first choice for model comparisons. \hmm{I would just omit this very bold last sentence.}
...
We note that metrics build upon equilibrium assumptions fail to meet criterion \enumref{enum:general}
...
\hmm{The footnote does not connect to the previous sentence but the the next one.}
...
`Turnover time' is an interesting example as it violates either criterion \enumref{enum:measurable} or criterion \enumref{enum:general} \hmm{Or both?}, 
...
If we implemented a function applying such a definition, it would not be general, and therefore its interpretation would be project-specific.
...
that are general and unambiguous, thus leaving it for the users to implement and justify additional assumptions or approximations used for their desired metrics. \hmm{Last word: ``quantities'' instead? Because it's not proper metrics following our criteria.}
...
\hmm{I recommend not to mention entropy at all. I'm just in the process of generalizing path entropy to nonautonomous systems, but this is still a bit ambiguous in my head and I need to think about it a bit more. There are definitely ways to compute the path entropy for a nonautononmous system such we could call it a metric, same for entropy rates.}

Surprisingly, the literature even contains proposals for metrics that cannot be defined as functions because they are ambiguously defined based on the following matrix factorization approach.  
...
\begin{eqnarray} 
  ...
  \label{eq:AxiK}
\end{eqnarray}
\hmm{Actually, $\xi$ is often considered a time-dependent environmental rate modifier (real value)}.
...
written in the form \eqref{eq:LinCompartmentalSystem} or \eqref{eq:AxiK}. This is relevant since such results have been published without discussion of the inherent ambiguity. We show in detail why this is the case in \Appendixref{appendix:MatrixDerivation}. \hmm{What is actually the case?}

\todo{@Markus In the following paragraph the 1st sentence doesn't flow...rephrase a bit? Besides, I'm not sure all what you list actually fulfills all the 3 criteria. E.g. contents of pools and fluxes between the pools may not be measurable at all,
...
It does not even make sense to ask the question otherwise. \hmm{Which question, I cannot see any?} \todo{@Markus quazi-equilibria as approximations? Can we mention something about that?}
...
obtained by the \LAPM{} package. \hmm{From now on the notation gets messy again. I corrected just a few things.} The equilibrium $x^\ast$ is defined by the
...
\X^* = - ({M^\ast})^{-1} \, \I^\ast.  
\end{equation}
\hmm{Why does $I$ get an asterisk here?}
...
time distributions do not. \hmm{The amount of what is changed?}
...
for the transit time distribution \citep{Metzler2018MGS}. \hmm{NOTATION!!!!!, B, M, u, I, x, X!!}
...
The \CompartmentalSystems{} package provides all the functionality to do these computations, which rely on a description of the time-dependent input vector $u(t)$ \hmm{NOTATION!} and the compartmental matrix $B(t)$ \hmm{NOTATION!}, as well as on the initial age distributions for the compartments.
...
x_0)$ back into it, which results in $\tilde{M}(t)=B(t, \X(t, t_0,\X_0))$ \hmm{That's my favorite equation so far, notation wise.} and
...
\hmm{In general, something seems to have happened to the citep command, it seems to have been replaced everywhere. So be careful when to use cite and citep. I'm trying to correct it, but I will surely miss some occurrences.}

\section{Software description}
\section{The \python{} packages}
\label{sec:PythonPackages}
\subsection{\LAPM{}}
Linear Autonomous Pool Models (\LAPM{}) is a \python\ 3 package for the study of autonomous compartmental systems in equilibrium such as those described in \sectionref{sec:Equilibrium}. 
...
following the strategy described in \sectionref{sec:trajectory}, computes the
...
This module uses \LAPM{}. \hmm{Which module is that?}
...
Instances consist of a set of variables with unique type (only one variable per type) \hmm {Or one type per variable?} and a set of type-annotated functions that exclusively 
...
\hmm{General question: If I define two identical models, just changing the pool order, can we automatically figure out that they are identical?}

It \hmm{What is ``it''?} also confines what we consider a model property, building block, or
...
\begin{enumerate}
  ...
  more result types can be reached. This is a forward graph search (\figref{fig:closure}).
    \begin{figure}[h]
      ...
      information is used to automatically add methods to a \texttt{CMTVS}
      ...
      mean age solution for the vegetation carbon subsystem which appears as \texttt{get_NumericVegetationCarbonMeanAgeSolutionArray()}.
      It is also the basis for queries, \eg ``for which
      models can we compute the mean transit time through
      the vegetation subsystem?''  Note that the applicable functions (in this
      example 45, represented by the gray nodes) can also be used
  ...
  computed via the function represented by the gray nodes. \hmm{What is not provided?}
  ...
  \texttt{StartConditionMaker} \footnote{Variables of this type are actually functions that can compute a set of consistent start conditions (mass, age density, mean age), which we note here to show that the concept of type is not confined to traditional data types.  
  } and 
...
\begin{figure}[h]
  ...
  \caption{Dependency graph}
  {
    \small
    \input{TypeLegend}
    \hmm{What about the f nodes in the caption?}
  }  
  \label{fig:dep_graph}
\end{figure}  
\end{enumerate} 
...
DSL does not have to provide everything and can restrict it \hmm{What is ``it''?} to sensible categories
for comparisons.
...
In particular \texttt{bgc\_md2} provides:
  \begin{enumerate}
    \item
      ...
      \footnote{A complete list can be produced by a single command. \hmm{Which command would that be?}}
  and is added to \appendixref{appendix:listing}
      ...
      by solving the parameterized IVPs \hmm{Has this abbreviation been introduced already?} and the most general diagnostics, \eg transient mean-age and transit time solutions. \hmm{The previous sentence is very long and confusing.}
      ...
      Assuming that a compartmental system has been defined symbolically, \eg by providing expressions for input, output, and internal fluxes, different flow diagrams can be created automatically. 
      The additional information for the decomposition into subsystems just consists of a set of pool names for each subsystem.
      (Here vegetation and soil part for the visit model reconstruction) \hmm{I don't get the previous sentence}.
      For special subsystems like vegetation or soil which frequently occur in carbon cycle models a declaration of a set of, \eg soil pools, has far reaching consequences. 
    ...
    \item
      A set of type annotated functions (called computers) operating on those types, which combined
    ...
    \item
    A collection of $30+$ vegetation, soil, or ecosystem models for carbon and nitrogen cycling
      as reusable \python\ modules using the building blocks in a flexible way. 
  \end{enumerate}

\subsubsection{Intentionally missing computers}
\todo {Comment from Kostia: I'm not sure this paragraph is necessary the way it is framed. There is already an idea in the previous paragraph that non-universal things should not be in 'computers'.
I don't think we should dwell on this too much. Unless it can be re-phrased in a positive manner, e.g. \"Although certain features are not implemented in computers, they are demonstrated in the example notebooks...\" In this case it can be merged with the following Section 4}
...
as we mentioned above and explained in detail in \Appendixref{appendix:MatrixDerivation}. 
...
\hmm{I have to admit that I basically do not understand much anymore from here on.}
...
\item 
  ...
  Fig. 7: Ensebmle mean is not visible in the image}
  (\figref{fig:btt_4models}, \figref{fig:stock_mean}) \hmm{I couldn't find this part in the notebooks. This is what people probably would like to see.}
\end{enumerate}
We will look closer at  
  \enumref{enum:trendy} It is interesting to see how only the same ten variables 
...
One special case is the computation of "pool" ages (age w.r.t. entry into a pool not the whole system) 
 ...
 \hmm{If I understand right, we can compute the ages of arbitrary subsystems more elegantly than done in the PNAS paper? That does not seem to be worth a publication to me.}
 
\hmm{Is the next part still comment or already manuscript text? I ignore it for now and continue with ``Conclusions''.}
...
\begin{figure}[t] \includegraphics[width=\columnwidth]{test_veg_soil.pdf}
  \caption{
    ...
    subsystems, across different models. \hmm{What is plotted? Medians? Means?}
  } 
...
\begin{figure}[t] \includegraphics[width=\columnwidth]{test_stock_mean.pdf}
  \caption{
    ...
    belong to which subsystem. \hmm{The titles of the subplots could be improved.}
  }
...
\hmm{A ``Conclusions'' section should be short. All the subsections that follow should go to a section called ``Discussion'', which is completely missing so far (should be between ``Results'' and ``Conclusions'').}
\hmm{I did not see any code for a transit time or age computation. Further, there was no explanation as to how add models to the database. This actually happens only later, but should definitely come AFTER the conclusion.}
We implemented a
...
are to our knowledge unprecedented.  In the process \hmm{In the process of what?} some lessons have been
...
and allows us to be strict in the database part \hmm{What do you mean by ``part''?} without creating a bottleneck.
...
When you are happy with your code \hmm{Tha's too informal.} a pull request will put your model into the default location 
...
\hmm{The next sentences is half a bullet point list and a sentence.}
This has advantages:
\begin{itemize}
  \item
...
\hmm{I'm not sure this is publication as a software package then - referring to the documentation. Should we write this here?}

\subsubsection{Polished UI, web interface}
\hmm{The table notebook has no comments and is basically useless this way. Furthermore, I no not think we should emphasize all the time what is totally not ready yet and can be done by others.}
...
\texttt{igrap} \hmm{Is it ``igraph``} support only basic graph layout algorithms.  
...
\hmm{Not all influxes and internal fluxes need to be strictly positive. Why is this requirement then missing for the outfluxes?}
\begin{align*}
%\X  &=(x_1,\dots x_n)^T
%\\
\mathcal{F} &=
...
The influxes do not have to depend on the $x_i$  but internal and outfluxes must at least depend \hmm{multiplicatively?} on their source pool content to guarantee the 
...
$F \in \mathcal C^1$, \hmm{This means $F$ is one time continuously differentiable, not just continuity. I think we need the stronger one.} we can write them in product form,
$F_{i \rightarrow j} = r_{j,i} x_i \text{ for } i \in \{1, \dots n\} , j \in \{ 0, 1,\dots ,n\}, \text{ and } j \ne i $. Then
\hmm{Why introduce $r$ now, when it becomes $M$ anyway? Why is it $r_{t \rightarrow}$ and not $r_{i \rightarrow 0}$? Same for $I$, where did the $0$ go? Later we even have $r_{i\rightarrow *}$. I'm heavily confused and cannot continue checking the formulas with unclear notation.}
\begin{align}
  \frac{d}{d t} x_i 
    &= - \underbrace{
      \left(
...
\hmm{Why is the compartmental matrix $-M$ now? It was already $B$ and $M$.}
...
\hmm{I think ''either or`` is wrong here. Furthermore, I think it just becomes too technical allowing I to depend on X and still call the compartmental system linear, as mentioned earlier.}
...
\hmm{$u$ has been used very differently early in the manuscript.}
...
Both  exclude certain compartmental models e.g. some with interactions between chemical species. \hmm{I do not understand the previous sentence.}
...
state vector, which makes it nonlinear. \hmm{Try to build a sentence around the equation.}
\begin{align*}
...
\hmm{Actually, $\xi$ is often real-valued, for instance in \texttt{smooth\_reservoir\_model.py:xi\_T\_N\_u\_representation}.}
...
From this representation it is clear that the $a_{i,j}$ \hmm{or $a_{j,i}$?} are only constant if all rates $r_{j,i} \text{ for } j \in \{0,\dots ,n \}$ contain the \emph{same} time dependent factor $\xi(t)$, which makes the existence of constant $A$ and $K$ 
an \emph{assumption}.
...
\hmm{Matrices used to be in capital letters, why now $d$?}
...
\ref{xi_examples}
\hmm{Here's a reference not working.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hmm{I don't know where they are hidden, but the ``Acknowledgements'' need to be done. Probably also the author contributions.}
