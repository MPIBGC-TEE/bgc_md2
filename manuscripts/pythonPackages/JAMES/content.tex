...
\section{Introduction}
\subsection{Outline}
\mm{Kostia and Holger}{
  This section had been missing which made the following sections almost
  unreadable. (partly also due to the different purpose they served in Carlos's
  original draft, from which I pasted them, without removing the duplication
  ...) I have completely rearranged the content now along the main argument.
}
...
\mm{all}{remove itemization when the sections are properly arranged}
\begin{itemize}
  \item
    In \sectionref{sec:ConceptualFramework} We define the scope by choosing 'Compartmental models' as the most general class of model we consider in terms of their 
    \begin{enumerate}
      \item underlying assumptions, 
      \item significance, i.e the possible questions that can be asked \mm{all}{(citations)}
      \item equivalent ways of representations, and 
      \item solution methods 
    \end{enumerate}
    Some of the details will be relegated to \appendixref{appendix:MatrixDerivation}.
  \item 
    In \sectionref{sec:} we briefly discuss the limits of the concept of equilibria and fixed points in the general context its approximations and generalizations since this provides the context for many traditionally and newly proposed less general metrics.
  \item
...
Hence, we proceed with a scientific discussion on what is to be regarded as a proper metric in \sectionref{sec:criteria} and formulate criteria for by which we evaluate proposed properties for intercomparison.
  \item
    We present all implemented metrics in the 
    context of limited their applicability. 
  \item 
    In \sectionref{sec:Intercomparison} we briefly describe the state of intercomparison frameworks
\end{itemize}
...
\figref{fig:modelGraph} represents compartments $A$ and $B$ by circles and the flux from $A$ to $B$ as an arrow with the annotation $F_{a\rightarrow b}{t,\bar{A},\bar{B},...}$   
...
\mm{mm}{Check that the citations all refer to compartmental models as the original text was:
...
%hydrology \cite{Nash1957IASH, Botter2011GRL, Harman2014GRL}, 
biogeochemistry
...
We show in detail in \appendixref{appendix:MatrixDerivation} how this intuitive graph can be translated into a system of ordinary differential equations in matrix form. 
...
%\mm{all}{The following should go into the appendix or removed if it is already there:
%If we assume any arbitrary ordering of the pools we can represent the mass (content of the pools)
%by an ordered tuple $\vec{x}$.
%Because mass is a non-negative quantity, this tuple of mass contents can
...
%\end{enumerate}
%\kv{Markus}{ Please add the description of notations you use in formulas throughout the paper}
%\mm{Kostia}{What concretely do you mean by that? Do you think there should be an index of notation here?
%I have started a glossary that we could make part of the document. 
%}
...
\begin{enumerate}
  ...
  \item
    \label{enum:general}
    \hmm{
      "Preferably" Should we use this word in a hard criterion?
    }
    \mmh{
      The second and third criteria are not as hard as the first one, which is really an excluding condition.
      We \emph{can} compute more specific metrics, but they are not the \emph{first} choice.
    }
  ...
  \item
    ...
    \hmm{"Preferably" Same question.}
    \mmh{
      Since the pools of many models are not well defined in a physically
      measurable sense we cannot even compare pool contents with real-world
      measurements. If we make this a \emph{hard} criterion it excludes almost
      \emph{every} soil model
    }
...
\hmm{Where exactly is this done? In the notebooks? In which one?}
...
\mm{Kostia}{references work if pdflatex is run twice...}



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Time Evolution} \label{sec:trajectory}
\mm{@all}{This whole subsection is a leftover from Carlos's first draft. It would only make sense to keep it if we use the state transition operator $\Phi$ later in some formulas}
...
\subsection{Compartmental systems in equilibrium} \label{sec:Equilibrium}
 \kmm{quasi-equilibria as approximations? Can we mention something about that?}
 \mmk{rewrote the whole paragraph subdividing it to deal with the different cases, including the quasi-static approximation}
...
\subsubsection{Linear autonomous systems}
...
\subsubsection{Non-linear autonomous systems}
...
However if the RHS does depend on time, many things can change `in an instant'. Not only will the roots of the RHS change position while the system is moving towards them, but the whole concept of a predictable destination collapses.
...
So criteria \enumref{enum:function} and \enumref{enum:general} are fulfilled. 
...
Formulas for the fluxes  either have preceded the matrix description or can be reverse engineered from it. They are fully determined by the trajectory of the pool contents and thus fulfill criteria \enumref{enum:function}.
...
Both age and transit time distributions can be unambiguosely computed for every pool at every time point, given inital distributions at the start of the simulation, so that criteria \enumref{enum:function} is fulfilled.
They are applicable to general nonautonomous nonlinear systems, thus fulfilling criteria \enumref{enum:general}.
Fullfillment of criteria \enumref{enum:measurable} is again conditional on a measurable destinction between pools, again possibly necessitating the definition of aggregate pools.
...
\mm{Kostia}{Unfortunately I do not know a single experiments where an age distribution for a nonautonomous system out of equilibrium has been actually been measured \emph{directly}. } 
...
Concludingly ages and transit-times of the whole system or physically distinguishable subsystems fulfill the conditions as well as contents or fluxes, placing them in the catagory of most desirable metrics so far.\mmh{Changed the wording and made the argument more specific}
...
General compartmental systems can be nonautonomous, in this case making the concept of equilibrium itself meaningless.
We note that metrics build upon equilibrium assumptions fail to meet criterion \enumref{enum:general}.
They can also be nonlinear, making the computation of the possibly empty set of fixed points by far to difficult to attempt automatically by a function, failing criterion \enumref{enum:function}.
\footnote{
...
`Turnover time' is an interesting example as it violates at least one of criteria \enumref{enum:measurable} and \enumref{enum:general}. 
...
\mmh{I didm't consider the word metric strictly defined as fulfilling our criteria but as some proposed numerical measure, to compare models by. Is there any part of the text left , where I particularly created this misleading impression?}
...
We show in detail why this is the case in \appendixref{appendix:MatrixDerivation}.
...
actually suggesting them based on argument types) for a DSL \glref{glossary:DSL} tailored to the domain of compartmental
...
In particular \texttt{bgc\_md2} provides:
  ...
  %and is added to \appendixref{appendix:listing}
    ...
    \item
      ...
      since they sometimes do not exist, or metrics depending directly or indirectly on ambiguously defined matrix factorizations \appendixref{appendix:MatrixDerivation}, since the result would be ambigous.  
      \footnote{
