...
We show in detail in \appendixref{appendix:MatrixDerivation} how this intuitive graph can be translated into an equation system in matrix form. 
...
This approach \emph{cannot}be extended to our most general situation of non-autonomuous nonlinear systems, the most obvious reasons being firstly the impossibility to even \emph{define} ``equilibrium'' or ``fixed point'  for non-autonomous systems and secondly the impossibility to assure the existence of fixed points for non-linear systems even if they are autonomous. We will explore these limitations and \emph{some} approaches to either approximate equilibrium or alltogether replace the concept.  
\subsubsection{Linear autonomous systems}
This is the traditional \emph{least} general case and the only one
where we can acertain the existence of an equilibrium based on some addtional assumbptions.
...
\begin{enumerate}
  \item
    \label{enum:function}
    In our definition we require that a metric must be unambiguously computable by a function of other metrics or building blocks of a model.    This is a exclusive condtion since it is not only essential to \emph{implement} a software tool but also to interpret its results without knowledge of model specific idiosyncrasies.
  \item
    \label{enum:general}
    Preferably 
    \hmm{Should we use this word in a hard criterion?}
    \mmh{The second and third criteria are not as hard as the first one, which is really an excluding condition.
    We \emph{can} compute more specific metrics, but they are not the \emph{first} choice.}
    metrics should be applicable to all (nonautonomous, nonlinear) compartmental systems, i.e. not rely on e.g. equilibrium conditions. While some metrics describe models under special conditions we want metrics that can be applied to as many models as possible. 
  \item
    \label{enum:measurable}
  Preferably 
 \hmm{Same question.}
    \mmh{Since the pools of many models are not well defined in a physically measurable sense we cannot even compare pool contents with real-world measurements. If we make this a \emph{hard} criterion it excludes almost everything... }
    metrics should also be physical properties i.e. have a clearly
